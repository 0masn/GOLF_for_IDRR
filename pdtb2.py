...
class CorpusReader:
    ...
    def iter_data(self, display_progress=True):
        df = pd.read_csv(self.src_filename, delimiter='\t', encoding='utf-8')
        header = df.columns.tolist()

        for i, (_, row) in enumerate(df.iterrows()):
            row_values = row.tolist()  # Convert pandas Series to list
            # print(row_values)
            if display_progress:
                sys.stderr.write("\r")
                sys.stderr.write("row {}".format(i + 1))
                sys.stderr.flush()
            row_values = ['' if pd.isna(
                value) else value for value in row_values]
            yield Datum(row_values, header)
        if display_progress:
            sys.stderr.write("\n")
...
class Datum:
    ...
    def __init__(self, row, header):
        """
        Initialize the Datum object with a row and header.
        """
        # Set the attributes.
        for i in range(len(row)):
            ...
            row_value = row[i]
            # if att_name == 'Section':
            #     print("ROW ATTR: ", att_name)
            #     print("ROW VAL: ", row_value)
            ...
            # Integer-valued.
                ...
                try:
                    ...
                    try:
                        ...
                        # Handle the case where it's neither a float nor an int.
                        pass
            # Trees.
            elif re.search(r"Trees", att_name):
                row_value = self.__process_trees(row_value)
            # The rest are strings and don't require special handling.
            elif pd.isna(row_value):
                row_value = None
    ...
    # def __pos(self, text, wn_format=False, lemmatize=False):
    #     """
    #     Internal method used to get the POS version, potentially lemmatized, associated with
    #     text strings (Arg1, Sup2, etc.)
    #     """
    #     results = []
    ...
    def relative_arg_order(self):
        ...
        raise Exception(
            "No relation could be determined for the two arguments!\n%s" % self.FullRawText)
    ...
    def to_graphviz(self, include_ptb=True):
        ...
        try:
            ...
            raise Exception("Can't find the Graphviz template file %s" %
                            GRAPHVIZ_TEMPLATE_FILENAME)
        # Title.
        new_line_removal = re.compile(r"\n\s*", re.M)
        cleaned_text = new_line_removal.sub(" ", self.FullRawText)
        title = '"%s\\n%s\\nSource: %s/wsj_00%s"' % (
            self.Relation, cleaned_text, self.Section, self.FileNumber)
    ...
    def __process_span_list(self, s):
        ...
        seqs = list(
            map((lambda x: map(int, re.split(r"\s*\.\.\s*", x))), parts))
        return seqs
